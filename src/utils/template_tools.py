from utils.embedding_provider import MilvusCollection
...
def alignment_callback(query_collection, query_id, target_collection, target_id):
    if target_collection == MilvusCollection.assembly_collection:
        return None
    if query_collection == MilvusCollection.assembly_collection:
        return None
    query_ref = 'af'
    if query_collection == MilvusCollection.instance_collection:
        query_ref = 'pdb'
    target_ref = 'af'
    if target_collection == MilvusCollection.instance_collection:
        target_ref = 'pdb'
    return f"alignment_callback('{query_ref}', '{query_id}', '{target_ref}', '{target_id}')"
